...
void custom_handle_mqtt_event_connected(esp_mqtt_event_handle_t event)
{
    ...
    msg_id = esp_mqtt_client_subscribe(client, CONFIG_MQTT_SUBSCRIBE_OTA_UPDATE_PATIO_GATE_TOPIC, 0);
    ESP_LOGI(TAG, "Subscribed to topic %s, msg_id=%d", CONFIG_MQTT_SUBSCRIBE_OTA_UPDATE_PATIO_GATE_TOPIC, msg_id);
}
...
void custom_handle_mqtt_event_data(esp_mqtt_event_handle_t event)
{
    ...
    if (strncmp(event->topic, CONFIG_MQTT_SUBSCRIBE_OTA_UPDATE_PATIO_GATE_TOPIC, event->topic_len) == 0)
    {
